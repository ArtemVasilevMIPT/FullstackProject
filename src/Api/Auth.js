...
const registerUser = (email, password, password_repeat) => {
    if (password != password_repeat) {
        //Panic
        return Promise.reject("Passwords don't match");
    }
    const registerBody = {email: email, password: password, nickname: email};
    return tokenRequest.post('/api/user/create/', registerBody)
        .then((response) => {
...
const loginUser = (username, password) => {
    ...
    return tokenRequest.post(`/api/user/auth/`, loginBody)
      .then((response)=> {
        window.localStorage.setItem(ACCESS_TOKEN, response.data.token.access);
        window.localStorage.setItem(REFRESH_TOKEN, response.data.token.refresh);
        return Promise.resolve(response.data);
      }).catch((error)=>{
        return Promise.reject(error);
      });
...
const checkAuth = () => {
    ...
    authRequest.get('/api/user/update/')
    .then((response) => {
        return Promise.resolve(response.data);
    }).catch((error) => {
        return Promise.reject(error);
...
export { tokenRequest, loginUser, logoutUser, registerUser, refreshToken, authRequest,
        errorInterceptor, checkAuth, BASE_URL, ACCESS_TOKEN, REFRESH_TOKEN }